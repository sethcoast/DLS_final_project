...
class Solver(object):
	...
	def train(self):
		# Setting parameters for the batch size, num of epochs
		loss = {}
		...
		for epoch in tqdm(range(self.resume_epoch, self.num_epoch)):
			...
			for i, (x_real, noise, label) in enumerate(tqdm(self.train_loader)):
				...
				if epoch * iter_per_epoch + i >= self.lr_decay_start:
					...
					if i % 1000 == 0:
						print('d_lr / g_lr is updated to {:.8f} / {:.8f} !'.format(self.d_optimizer.param_groups[0]['lr'], self.g_optimizer.param_groups[0]['lr']))
				...
				# Step:1 Generate a fake image from the generator and try to fool the discriminator
				...
				# sample
				# Step:2 Supervised training of the discriminator
				# Take a fake image and real image and classify it
				try:
				...
				# =================================================================================== #
				# NOTE: While training the generator keep discriminator training off
			...
			print("Generator, discriminator, and total loss for {} epoch is: {}, {}, {}".format(epoch, my_gen, my_disc, my_gen+my_disc))
